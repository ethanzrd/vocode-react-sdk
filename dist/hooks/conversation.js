...
const useConversation = (config) => {
    ...
    const [latencyRecords, setLatencyRecords] = react_1.default.useState([]);
    ...
    const startConversation = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        socket.onmessage = (event) => {
            ...
            if (message.type === "latency_data") {
                setLatencyRecords(prev => [...prev, message.data])
            }
            if (message.type === "websocket_audio") {
                ...
                setTranscripts((prev) => {
                    // Create a copy of the last transcript
                    let prevCopy = [...prev];
                    let last = prevCopy.pop();
